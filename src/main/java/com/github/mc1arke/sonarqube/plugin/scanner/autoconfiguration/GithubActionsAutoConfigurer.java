...
public class GithubActionsAutoConfigurer implements BranchAutoConfigurer {
    ...
    public Optional<BranchConfiguration> detectConfiguration(System2 system, ProjectBranches projectBranches) {
        ...
        String branch = system.envVariable("GITHUB_REF");
        if (StringUtils.isEmpty(branch)) {
            return Optional.empty();
        }

        String sourceBranch = system.envVariable("GITHUB_HEAD_REF");
        if (branch.startsWith(PULL_REQUEST_REF_PREFIX) && branch.endsWith(PULL_REQUEST_REF_POSTFIX) && StringUtils.isNotEmpty(sourceBranch)) {
            String key = branch.substring(PULL_REQUEST_REF_PREFIX.length(), branch.length() - PULL_REQUEST_REF_POSTFIX.length());
            ...
            return Optional.of(branchConfigurationFactory.createBranchConfiguration(branch.substring(12), projectBranches));
        }
    }
}
